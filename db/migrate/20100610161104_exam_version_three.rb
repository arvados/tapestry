class ExamVersionThree < ActiveRecord::Migration
  ...
  def self.down
    ...
    e = ExamVersion.find_by_title_and_version('Risks and Benefits',3)
    ...
    e = ExamVersion.find_by_title_and_version('Transmission',3)
    e.destroy unless e.nil?
    v2 = ExamVersion.find_by_title_and_version('Transmission',2)
    if not v2.nil? then
      v2.published = true
    ...
    e = ExamVersion.find_by_title_and_version('Gene Expression and Regulation',2)
    e.destroy unless e.nil?
    v2 = ExamVersion.find_by_title_and_version('Gene Expression',1)
    if not v2.nil? then
      v2.published = true
    ...
    e = ExamVersion.find_by_title_and_version('Genetics & Society',2)
    e.destroy unless e.nil?
